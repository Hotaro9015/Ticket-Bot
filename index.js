...
client.on("interactionCreate", async (btn) => {
  if (btn.values == "stop_client") {
    if(!client.config.developers.includes(btn.member.id)) return btn.reply({
      content: "<:cross:983304450787659776> You can't use this!",
      ephemeral: true
    })
    try {
      btn.reply({
        content: "<a:loading_:983301946217725982> **Shutting down the bot...**",
        ephemeral: true
      })
      setTimeout(() => {
        process.exit()
  ...
  if (btn.values == "rename_client") {
    ...
    btn.reply({ content: "<a:loading_:983301946217725982> **Send new Bot Name**",
        ephemeral: true})
    ...
    collector.on("end", (collected) => {
      ...
      if (!name) {
        return btn.reditReply({ content: `<:cross:983304450787659776> **No Name!**`})
      }
      let beforename = client.user.username;
      client.user.setUsername(name)
        .then((user) => {
          message.delete()
          btn.editReply({ content: `<a:yes:983304528596172830> **Succesfully set name to ${client.user.username} from ${beforename}**`,
        ephemeral: true})
        })
        .catch((e) => {
  ...
  if (btn.values == "changeav_client") {
    ...
    btn.reply({ content: "<a:loading_:983301946217725982> **Send the New Bot Image**",
        ephemeral: true})
    collector.on("collect", async (msg) => {
        let url = msg.content;
      if(msg.content.includes(`https://`)) {
        btn.editReply({ content: "<a:loading_:983301946217725982> **Changing avatar...**",
        ephemeral: true})
        ...
        client.user.setAvatar(url)
          .then(user => {
              ...
              btn.editReply({ content: "<a:yes:983304528596172830> **Succesfully changed the Bot's avatar!**",
        ephemeral: true})
        ...
        msg.delete()
        btn.editReply({ content: "<:cross:983304450787659776> Not a valid URL",
        ephemeral: true})
      }
    })
  }
